...
The root layout conditionally injects the Plausible script when `NEXT_PUBLIC_PLAUSIBLE_DOMAIN` is set. When the WhatsApp buttons are clicked we emit a `TAP_WHATSAPP` custom event with the instructor ID, enabling funnel tracking in Plausible.
